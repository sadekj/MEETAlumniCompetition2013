...
(function($) {
	$._spritely = {
			...
			if (options.type == 'sprite') {
				...
				var animate = function(el) {
					...
					el
							.css(
									'background-position',
									frames[$._spritely.instances[el_id]['current_frame']]
											+ 'px 0');
					if (options.bounce && options.bounce[0] > 0
							&& options.bounce[1] > 0) {
				...
				if ($.browser.msie) {
					...
					$(el).css('background-position',
							$._spritely.instances[el_id]['l'] + 'px ' + bp_top);
				} else {
		...
		randomIntBetween : function(lower, higher) {
			return parseInt(rand_no = Math.floor((higher - (lower - 1))
					* Math.random())
					+ lower);
		}
	};
	$.fn.extend({
		...
		flyToTap : function(options) {
			...
			if ($._spritely.activeSprite) {
				if (window.Touch) { // iphone method see
									// http://cubiq.org/remove-onclick-delay-on-webkit-for-iphone/9
									// or
									// http://www.nimblekit.com/tutorials.html
									// for clues...
		...
		activeOnClick : function() {
			...
			if (window.Touch) { // iphone method see
								// http://cubiq.org/remove-onclick-delay-on-webkit-for-iphone/9
								// or http://www.nimblekit.com/tutorials.html
								// for clues...
